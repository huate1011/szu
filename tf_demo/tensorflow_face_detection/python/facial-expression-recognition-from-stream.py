...
model = load_model("model_auto_gen_20_epochs.h5")
# -----------------------------
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        for i, data in enumerate(predictions[0] * 100):
            cv2.putText(img, "{}: {:2.2f}%".format(emotions[i], data), (int(x) + 200, int(y) + i*30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    ...
    if input & 0xFF == ord('q'):  # press q to quit
        break
    if input & 0xFF == ord('s'):
        time.sleep(3)
